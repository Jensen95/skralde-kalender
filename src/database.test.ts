...
describe('Database Operations', () => {
  ...
  describe('getEventsByAddress', () => {
    it('should filter events by address using LIKE', async () => {
      ...
      const events = await getEventsByAddress(mockD1 as any, 'Eksempelvej 1')

      expect(mockD1.prepare).toHaveBeenCalledWith(expect.stringContaining('WHERE location LIKE ?'))
      expect(mockStatement.bind).toHaveBeenCalledWith('%Eksempelvej 1%')
      expect(events).toHaveLength(1)
      expect(events[0].location).toBe('Eksempelvej 1, 1234 Bynavn')
    })
  })
